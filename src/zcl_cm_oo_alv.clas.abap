...
public section.
  ...
  methods MT_CREATE_OO_DW_ALV
    importing
      value(IV_REPID) type PROGNAME default SY-CPROG
      value(IV_SCREEN) type SY-DYNNR default '9000'
      value(IV_DEFAULT_EX) type ZE_CM_TOOLBAR_EXCLUDE_TYPE default 'S'
      value(IS_LAYOUT) type LVC_S_LAYO optional
      value(IT_FIELDCAT) type LVC_T_FCAT optional
      value(IT_EXCLUDE) type UI_FUNCTIONS optional
      value(IV_SCREEN_PERCENT) type I
      value(GV_SIDE) type I default '4'
      value(IV_VARIANT_HANDLE) type DISVARIANT-HANDLE default '1'
      !I_F4_FORM type STRING optional
      !I_TOOLBAR_FORM type STRING optional
      !I_USER_COMMAND_FORM type STRING optional
...
METHOD FREE.
  ...
  IF GO_DOCKING_CONT IS NOT INITIAL.
    CALL METHOD GO_DOCKING_CONT->FREE.
  ENDIF.
...
* create alv
  ...
  IF IS_LAYOUT IS NOT INITIAL.
    GS_LAYOUT = IS_LAYOUT.
  ELSE.
    GS_LAYOUT = ME->MT_GET_DEFAULT_LAYOUT( ).
  ENDIF.
...
*       ENDIF.
  ...
  CREATE OBJECT GO_EVENT
    EXPORTING
      ...
      I_PRINT_TOP_OF_PAGE         = I_PRINT_TOP_OF_PAGE
      I_F4_FORM                   = I_F4_FORM
      I_TOOLBAR_FORM              = I_TOOLBAR_FORM
      I_BEFORE_UCOMM_FORM         = I_BEFORE_UCOMM_FORM
      I_USER_COMMAND_FORM         = I_USER_COMMAND_FORM
      I_AFTER_UCOMM_FORM          = I_AFTER_UCOMM_FORM
      I_BUTTON_CLICK_FORM         = I_BUTTON_CLICK_FORM
      I_MENU_BUTTON_FORM          = I_MENU_BUTTON_FORM
      I_ALV_DRAG_FORM             = I_ALV_DRAG_FORM
      I_ALV_DROP_FORM             = I_ALV_DROP_FORM.
...
METHOD MT_CREATE_OO_DW_ALV.

  DATA LT_SORT TYPE LVC_T_SORT.
  DATA LT_FILTER TYPE LVC_T_FILT.

  DATA LT_FILEDCAT TYPE LVC_T_FCAT.
  DATA LS_FILEDCAT TYPE LVC_S_FCAT.
*  DATA LS_LAYOUT TYPE LVC_S_LAYO.
  DATA LT_EXCLUDE TYPE UI_FUNCTIONS.
...
* Create container
  CREATE OBJECT GO_DOCKING_CONT
    EXPORTING
      PARENT = CL_GUI_CONTAINER=>SCREEN0
      RATIO  = IV_SCREEN_PERCENT
      SIDE   = CL_GUI_DOCKING_CONTAINER=>DOCK_AT_BOTTOM. "DOCK_AT_RIGHT. " DOCK_AT_BOTTOM .
  IF SY-SUBRC <> 0.
...
ENDMETHOD.                    "MT_GET_DEFAULT_FIELDCAT
